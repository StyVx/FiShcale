...
async function createParty() {
  ...
  if (!uname) return toast('Enter your name!');
...
async function joinParty() {
  ...
  if (!code) return toast('Enter a party code!');
  if (!uname) return toast('Enter your name!');
  ...
  if (party.users?.[uname] && uname !== S.curUser) {
    toast('Name already taken!');
    return;
  }

  // ensure user exists
  if (!party.users?.[uname]) {
    await partyRef.child('users/' + uname).set({ history: [] });
  }
...
function subscribeToParty(code) {
  ...
  partyRef.off(); // 🔥 prevent duplicate listeners
...
async function confirmPick() {
  ...
  await db.ref(`parties/${S.curParty}/users/${S.curUser}/history`).push(entry);
...
function latestFish(u, p) {
  ...
  if (!party?.users?.[uname]?.history) return null;

  const hist = Object.values(party.users[uname].history);
